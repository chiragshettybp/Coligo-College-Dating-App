// ============================================================================
// /chat/:chatId/report — dedicated report page (never a popup). Choose a reason,
// add optional details, and submit to Supabase (reports table). Duplicate
// reports are prevented server-side (reporter/reported pairing).
// ============================================================================
...
import { chatInfoQuery, reportChatUser, REPORT_DETAILS_MAX } from "@/lib/chat.functions";
...
export const Route = createFileRoute("/_authenticated/chat/$chatId/report")({
...
function ReportPage() {
  ...
  const report = useServerFn(reportChatUser);

  const [reason, setReason] = useState<string | null>(null);
  const [details, setDetails] = useState("");
  const [busy, setBusy] = useState(false);

  if (!info) return <ReportUnavailable />;
  const name = info.other.fullName ?? "this person";
  const tooLong = details.length > DETAILS_MAX || details.length > REPORT_DETAILS_MAX;
  ...
  const onSubmit = async () => {
    ...
    try {
      const res = await report({
        data: { userId: info.other.id, reason, details: details.trim() || undefined },
      });
      await qc.invalidateQueries({ queryKey: chatInfoQuery(chatId).queryKey });
      toast.success(
        res.duplicate ? "You've already reported this person." : "Report submitted. Our team will review it.",
      );
  ...
  return (
    <DiscoverShell active="matches">
      <TopBar title="Report" onBack={() => navigate({ to: "/chat/$chatId", params: { chatId } })} />

      <div className="flex items-center" style={{ gap: spacing[3], marginTop: spacing[4] }}>
        <Avatar src={info.other.photo ?? undefined} initials={name.slice(0, 1).toUpperCase()} size="md" />
...
function ReportUnavailable() {
  ...
  return (
    <DiscoverShell active="matches">
      <TopBar title="Report" onBack={() => navigate({ to: "/chat" })} />
